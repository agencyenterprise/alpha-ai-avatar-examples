...
export function useLiveKitRoom({ videoRef, audioRef, token, serverUrl, onSpeechRecognized }: UseLiveKitRoomOptions) {
  ...
  useEffect(() => {
    ...
    function handleTrackSubscribed(track: RemoteTrack) {
      ...
      } else if (audioRef && track.kind === Track.Kind.Audio && room.canPlaybackAudio) {
    ...
    function handleDataReceived(data: Uint8Array) {
      const parsedMessage: ParsedMessage = JSON.parse(new TextDecoder().decode(data));
      ...
      if (parsedMessage.type === MessageType.Transcript && callbackRef.current) {
